...
import torch.nn.functitune_ibonal as F
...
def mask_noise( x, edge_index,mask_rate):
    num_nodes = len(x)
...
def train_mae(args, model_list, loader, optimizer_list, device, alpha_l=1.0, loss_fn="sce"):
    ...
    model, mlp1,mlp2,dec_pred_atoms, dec_pred_bonds = model_list
    optimizer_model, optimizer_MLP1,optimizer_MLP2,optimizer_dec_pred_atoms, optimizer_dec_pred_bonds = optimizer_list
    '''
    1.
    model, dec_node_edge = model_list
    optimizer_model_PIB, optimizer_dec_node_edge = optimizer_list
    model_PIB.train()
    dec_node_edge.train()
    ...
    for step, batch in enumerate(epoch_iter):
        ...
        noise_node_rep=node_rep[masked_node_indices]
        H=mlp1(node_rep)
        noise_H=mlp2(noise_node_rep)
        ...
        if args.mask_edge:
            ...
            #print('node_rep.shape:',node_rep.shape)
...
def main():
    ...
    dataset = MoleculeDataset("/Chem/dataset/" + dataset_name, dataset=dataset_name)
    ...
    '''4.
    model_list = [model_PIB, node_edge_decoder]
    '''
    ...
    '''5.
    optimizer_list = [optimizer_model_PIB, optimizer_node_edge_decoder]
    '''
    output_file_temp = "/Chem/checkpoints/" + args.output_model_file + f"_{args.gnn_type}"
    ...
    output_file = "/Chem/checkpoints/" + args.output_model_file + f"_{args.gnn_type}"
    if resume:
