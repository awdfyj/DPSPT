...
class BatchMasking(Data):
    ...
    def from_data_list(data_list):
        ...
        #torch.cat(batch['edge_index'],dim=1)
        #print('edge_index,success')
        #torch.cat(batch['edge_org'],dim=1)
        #print('edge_org,success')
        #print('data_list[0].__cat_dim__(key, batch[key][0])',data_list[0].__cat_dim__('edge_index', batch['edge_index'][0]))
        for key in keys:
                #torch.cat(batch[key],dim=0)
                #print('batch[key]',batch[key])
                #print('data_list[0].__cat_dim__(key, batch[key][0])',data_list[0].__cat_dim__(key, batch[key][0]))
            if key=='edge_org':
                batch[key] = torch.cat(
                batch[key], dim= 1)
            else:
